                ...
                <div class="page-content">
                    <div id="tab-general">
                     <?php
                        ...
                        $product = $_POST['premio'];
                        $queryI= "INSERT INTO Premis_Usuari (ref_usuari,ref_premi,DataHora) VALUES ($id_Usuari,$product,'$time[0] $time2[0]')";
                        if(mysqli_query($db,$queryI)){
                            echo "<b>Premio Adquirido</b></br>";
                        }else{
                            echo "Problemas al adquirir el premio</br>";
                            ?><script>alert("Problemas al adquirir el premio");window.location = "gift.php"</script>;<?php
                            echo "Error: ".mysqli_error($db); 
                        }
                        $query="SELECT * from Premis WHERE id_premi = $product";
                        $db->query($query);
                        $id_res = $db->query($query);
        while($row = mysqli_fetch_array($id_res)){
            $coste= $row["cost"];
        }
        $restacoins = $coins - $coste;
                        $queryI= "Update Usuaris SET Coins =  $restacoins where Correu = '$user'";
                        if(mysqli_query($db,$queryI)){
                            echo "<b>Coins actualizadas</b></br>";
                            ?><script>alert("Compra Satisfactoria");window.location = "gift.php"</script>;<?php
                        }else{
                            echo "Problemas al adquirir el premio</br>";
                            echo "Error: ".mysqli_error($db); 
                        }
